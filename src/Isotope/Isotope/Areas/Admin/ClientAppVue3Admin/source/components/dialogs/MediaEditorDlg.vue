...
async function save() {
  ...
  // Auto navigate to next if enabled and there's a next item
  const shouldEditNext = editNext.value === true;
  if (shouldEditNext && canGoNext.value) {
    navigate('next');
  } else if (!shouldEditNext) {
    // Close the dialog only if checkbox is not checked
    isOpen.value = false;
  }
  // If checkbox is checked but no next item, keep dialog open (on last item)
}
