...
async function loadMedia(bustCache: boolean = false) {
  media.value = await api.media.getList(folderKey.value);
  if (bustCache) {
    const timestamp = Date.now();
    media.value.forEach(m => {
      m.thumbnailPath += (m.thumbnailPath.includes('?') ? '&' : '?') + 't=' + timestamp;
    });
  }
...
function onEditorSaved(result: boolean) {
  if (result) {
    // Reload media to get updated thumbnails (bust cache to refresh changed thumbnails)
    loadMedia(true);
  }
}
</script>
