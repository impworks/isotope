...
/// </summary>
public class SeedContext(AppDbContext db)
{
    private readonly Dictionary<string, int> _mediaOrder = new Dictionary<string, int>();
    ...
    {
        var foldersList = await db.Folders
                                   .AsNoTracking()
                                   .Include(x => x.Tags)
                                   .Where(x => x.Depth != 0)
                                   .ToListAsync();
        ...
        foreach (var m in media)
        {
            ...
            db.MediaTags.AddRange(tagIds.Select(x => new MediaTagBinding
            {
                TagId = x,
                MediaKey = m.Key,
